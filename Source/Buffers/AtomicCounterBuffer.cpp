...
static GLsync theThing = 0;
#include <stdio.h>

// TODO: header
void PersistentAtomicCounterBuffer::ResetCounter() const
{
    theThing = glFenceSync(GL_SYNC_GPU_COMMANDS_COMPLETE, 0);
    printf("value is %u\n", *_bufferPtr);
    *_bufferPtr = 0;
    while (glClientWaitSync(theThing, GL_SYNC_FLUSH_COMMANDS_BIT, 0) != GL_ALREADY_SIGNALED);
