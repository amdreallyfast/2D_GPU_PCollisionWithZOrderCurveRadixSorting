The intermediate sort buffers and PrefixScanBuffer::PrefixSumsWithinGroup buffer are the same size.
...
- launched with 1 thread for each item in PrefixScanBuffer::PrefixSumsWithinGroup
...
radix sort loop through 32 bits
{
    ...
    - puts the 0 or 1 in the PrefixScanBuffer::PrefixSumsWithinGroup 
    
    ParallelPrefixScanIntermediateData.comp
    - launch with half the number of threads as the size of the PrefixScanBuffer::PrefixSumsWithinGroup (the algorithm requires that each thread handle 2 items)
    
    ParallelPrefixScanWorkgroupSums.comp
    - launch with 1 work group worth of threads
    
    SortIntermediateDataUsingPrefixSums.comp
    - launch with 1 thread for each item in the PrefixScanBuffer
    - reads from the "read" buffer in IntermediateSortBuffers.comp
    - uses prefix sum of the thread's work group (PrefixScanBuffer::PerGroupSums) + the prefix sum within the thread's work group (PrefixScanBuffer::PrefixSumsWithinGroup) to calculate the destination index
...
- launched with 1 thread for each item in ParticleBuffer (NOT 1 for each item in PrefixScanBuffer::PrefixSumsWithinGroup like DataToIntermediateDataForSorting.comp did)
